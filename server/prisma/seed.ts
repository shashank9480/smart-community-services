...
async function main() {
  ...
  // 7. Seed Demo Visitor Pass & Parcel
  const pass1 = await prisma.visitorPass.create({
    data: {
      code: '849201',
      created_by: admin.id,
  ...
  console.log('✅ Created demo Visitor Pass (849201) & Delivery Parcel (OTP: 4829)');

  // 8. Seed Demo Invoices & Ledgers
  ...
  console.log('✅ Created demo ERP Maintenance Invoices');

  // 9. Seed Demo Helpdesk Tickets
  await prisma.ticket.create({
    data: {
      raised_by: admin.id,
      category: 'Plumbing',
      description: 'Water pipe vibration in Block A main riser column during peak morning hours.',
      status: 'In Progress',
    },
  });
  await prisma.ticket.create({
    data: {
      raised_by: admin.id,
      category: 'Electrical',
      description: 'Basement B1 pillar 12 light fixture flickering.',
      status: 'Open',
    },
  });
  console.log('✅ Created demo Helpdesk Tickets');

  // 10. Seed Demo Notices
  await prisma.notice.create({
    data: {
      society_id: society.id,
      posted_by: admin.id,
      title: '📢 Annual Society General Body Meeting (AGM 2026)',
      body: 'Dear Residents, our annual general body meeting will take place this Sunday at 10:00 AM in the Grand Clubhouse. Agenda includes solar rooftop panel installation and security upgrading.',
    },
  });
  console.log('✅ Created demo Society Notice');
