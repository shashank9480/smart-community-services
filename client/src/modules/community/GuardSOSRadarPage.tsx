  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200 text-slate-100 font-mono">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800 text-xs text-emerald-400 font-bold">
          <Radio className="w-4 h-4 animate-ping" /> RADAR ROOM: room:guards
        </div>
      </div>

      {activeAlerts.length > 0 && (
        <div className="p-6 rounded-2xl bg-red-950 border-2 border-red-600 shadow-2xl text-white animate-pulse space-y-4 font-sans">
          <div className="flex items-center gap-3">
            <ShieldAlert className="w-10 h-10 text-red-400 shrink-0" />
            <div>
              <h3 className="text-2xl font-black">🚨 ACTIVE EMERGENCY ALARM TRIGGERED!</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
            {activeAlerts.map((a) => (
              <div key={a.id} className="p-4 rounded-xl bg-slate-950 border border-red-800 space-y-2">
                <div className="flex items-center justify-between">
      ...
      <Card dark title="Emergency SOS Alarm Incident Audit Log">
        {loading ? (
          ...
          <div className="space-y-3 mt-4">
            {alerts.map((a) => (
              <div key={a.id} className="p-4 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-between font-sans">
                <div>
