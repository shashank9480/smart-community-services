...
export const Sidebar: React.FC<{ role: 'ADMIN' | 'RESIDENT' | 'GUARD' }> = ({ role }) => {
  ...
  const adminNav: SidebarItem[] = [
    ...
    { name: 'Residents Directory', href: '/admin/residents', icon: <Users className="w-5 h-5" /> },
    { name: 'Security Guards', href: '/admin/guards', icon: <ShieldCheck className="w-5 h-5" /> },
  ...
  const guardNav: SidebarItem[] = [
    ...
    { name: 'SOS Receiver', href: '/guard/sos-radar', icon: <Radio className="w-5 h-5 text-red-500 animate-pulse" />, badge: 'Live Radar' },
  ];
  ...
  return (
    <aside className={clsx(
      'w-64 flex-shrink-0 flex flex-col border-r min-h-screen select-none relative z-20',
      ...
      <div className="h-16 flex items-center gap-3 px-6 border-b border-slate-800/80 bg-slate-950/40">
        <motion.div
          ...
          whileTap={{ scale: 0.95 }}
          className="w-8 h-8 rounded-lg bg-emerald-600 flex items-center justify-center font-bold text-white shadow-md shadow-emerald-500/30 cursor-pointer"
        >
          SCS
        </motion.div>
        <div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <NavLink
              key={item.name}
              to={item.href}
              end={item.href === '/admin' || item.href === '/resident' || item.href === '/guard'}
              className={({ isActive }) =>
                clsx(
                  'relative flex items-center justify-between px-3 py-2.5 rounded-lg text-sm font-medium transition-colors duration-150',
                  isActive
                ...
                <>
                  ...
                  {item.badge && (
                    <motion.span
                      ...
                      className="relative z-10 px-2 py-0.5 text-[10px] font-mono font-semibold rounded bg-emerald-950 text-emerald-300 border border-emerald-800 shadow-sm"
                    >
                      {item.badge}
                    </motion.span>
                  )}
      ...
      <div className="p-4 border-t border-slate-800 bg-slate-950/60">
        ...
        <div className="space-y-2">
          <motion.button
            whileHover={{ scale: 1.02, backgroundColor: 'rgba(6, 78, 59, 0.5)' }}
            whileTap={{ scale: 0.97 }}
            onClick={() => setIsProfileModalOpen(true)}
            className="w-full flex items-center justify-center gap-2 px-3 py-1.5 rounded-lg text-xs font-semibold text-emerald-400 bg-emerald-950/30 border border-emerald-900/50 transition-colors"
          >
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            onClick={logout}
            className="w-full flex items-center justify-center gap-2 px-3 py-1.5 rounded-lg text-xs font-semibold text-red-400 bg-red-950/30 border border-red-900/50 transition-colors"
          >
